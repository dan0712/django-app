...
# This docker entrypoint was pulled directly from the stock wordpress image.
# https://raw.githubusercontent.com/docker-library/wordpress/master/apache/docker-entrypoint.sh
# It is needed as on bluemix we need the mounted volume below where the data is stored (ie at /var/www) so we can change
# the permissions on the directories, but the std wordpress image mounts a volume at /var/www/html, which splats the
# bluemix mount.
...
fwrite($stderr, "\n" . 'mysql args:' . $argv[1] . " " . $argv[2] . " " . $argv[3] . " " . $argv[4]);
$maxTries = 10;
do {
