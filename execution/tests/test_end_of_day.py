...
class BaseTest(TestCase):
    ...
    def test_change_account_cash(self):
        ...
        ib_account_cash[account.ib_account.ib_account] = 1000
        difference = reconcile_cash_client_account(account)
        self.assertAlmostEqual(0, difference)

        self.assertEqual(account.ib_account.ib_account, 'DU299694')

        #deposit - transferred to account.cash_balance
        ib_account_cash[account.ib_account.ib_account] = 1100
        ...
        ib_account_cash[account.ib_account.ib_account] = 900
        reconcile_cash_client_account(account)
